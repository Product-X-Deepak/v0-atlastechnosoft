...
import { ArrowRight, Factory, Bot, BarChart3, Settings } from "lucide-react"
import { Button } from "@/components/ui/button"
import Link from "next/link"
import { Suspense } from "react"

function ManufacturingHeroComponent() {
  const ref = useRef<HTMLElement>(null)
  const isInView = useInView(ref as React.RefObject<Element>, { once: true, amount: 0.3 })
  
  return (
    <section ref={ref} className="relative overflow-hidden bg-[#F0F7FF] py-8">
      <div className="container relative z-10 px-4 md:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-4 max-w-4xl text-center">
          <h1 className="mb-3 text-2xl font-bold tracking-tight text-slate-800 sm:text-3xl md:text-4xl">
            Manufacturing <span className="text-[#E84A0E]">Solutions</span> <br />
            <span className="text-[#A73370]">for Smart Factories</span>
          </h1>
          <p className="mx-auto mb-4 max-w-2xl text-base text-slate-700">
            Intelligent production management and automation solutions for the modern manufacturer with advanced SAP integrations.
          </p>
        </div>
...
function ManufacturingHero(props: Record<string, never>) {
  return (
    <Suspense fallback={<div className="min-h-[300px] animate-pulse bg-muted/20 rounded-lg" />}>
      <ManufacturingHeroComponent {...props} />
    </Suspense>
  )
}
...
const benefits = [
  {
    icon: <Factory className="h-4 w-4 text-[#E84A0E]" />,
    title: "Production Throughput",
    description: "Increase by up to 32%",
  },
  {
    icon: <Bot className="h-4 w-4 text-[#A73370]" />,
    title: "Unplanned Downtime",
    description: "Reduce by 45%",
  },
  {
    icon: <BarChart3 className="h-4 w-4 text-[#A73370]" />,
    title: "Manufacturing Costs",
    description: "18% decrease overall",
  },
  {
    icon: <Settings className="h-4 w-4 text-[#E84A0E]" />,
    title: "On-Time Delivery",
    description: "94% performance rate",
  },
]

// Wrapper component to ensure proper Suspense boundaries for useSearchParams
function ManufacturingHeroWrapper(props: Record<string, never>) {
  return (
    <Suspense fallback={<div className="min-h-[300px] animate-pulse bg-muted/20 rounded-lg" />}>
      <ManufacturingHero {...props} />
    </Suspense>
  );
}

// Modify the export to use the wrapped version
export { ManufacturingHeroWrapper as ManufacturingHero };