...
import { ArrowRight, ActivitySquare, Clock, FileCheck, HeartPulse } from "lucide-react"
import { Button } from "@/components/ui/button"
import Link from "next/link"
import { Suspense } from "react"

function HealthcareHeroComponent() {
  const ref = useRef<HTMLElement>(null)
  const isInView = useInView(ref as React.RefObject<Element>, { once: true, amount: 0.3 })
  
  return (
    <section ref={ref} className="relative overflow-hidden bg-[#F0FDF9] py-8">
      <div className="container relative z-10 px-4 md:px-6 lg:px-8">
        ...
        <div className="mx-auto mt-4 max-w-4xl text-center">
          <h1 className="mb-3 text-2xl font-bold tracking-tight text-slate-800 sm:text-3xl md:text-4xl">
            Healthcare <span className="text-[#E84A0E]">Solutions</span> <br />
            <span className="text-[#A73370]">for Digital Transformation</span>
          </h1>
          <p className="mx-auto mb-4 max-w-2xl text-base text-slate-700">
            Transform patient care and operations with advanced technology solutions designed for modern healthcare organizations.
          </p>
        </div>
...
function HealthcareHero(props: Record<string, never>) {
  return (
    <Suspense fallback={<div className="min-h-[300px] animate-pulse bg-muted/20 rounded-lg" />}>
      <HealthcareHeroComponent {...props} />
    </Suspense>
  )
}
...
const benefits = [
  {
    icon: <FileCheck className="h-4 w-4 text-[#E84A0E]" />,
    title: "Administrative Costs",
    description: "24% reduction",
  },
  {
    icon: <HeartPulse className="h-4 w-4 text-[#A73370]" />,
    title: "Patient Satisfaction",
    description: "27% increase",
  },
  {
    icon: <Clock className="h-4 w-4 text-[#A73370]" />,
    title: "Accounts Receivable",
    description: "18 days reduction",
  },
  {
    icon: <ActivitySquare className="h-4 w-4 text-[#E84A0E]" />,
    title: "Claim Rates",
...
function HealthcareHeroWrapper(props: Record<string, never>) {
  return (
    <Suspense fallback={<div className="min-h-[300px] animate-pulse bg-muted/20 rounded-lg" />}>
      <HealthcareHero {...props} />
    </Suspense>
  );
}

// Modify the export to use the wrapped version
export { HealthcareHeroWrapper as HealthcareHero };